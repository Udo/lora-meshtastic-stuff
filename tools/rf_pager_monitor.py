...
POCSAG_RE = re.compile(
    r"^(?P<mode>POCSAG\d+):\s+Address:\s*(?P<address>\d+)\s+Function:\s*(?P<function>\d+)\s+(?P<body>.*)$"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Live pager monitor using rtl_fm piped into multimon-ng POCSAG decoders")
    ...
    parser.add_argument("--frequency", type=int, default=169_650_000, help="Pager frequency in Hz")
    ...
    parser.add_argument("--max-addresses", type=int, default=18)
    parser.add_argument("--show-raw", action="store_true")
    parser.add_argument("--log-file", help="Optional JSONL log path for decoded pager messages")
...
def validate_binaries(args: argparse.Namespace) -> None:
    ...
    if shutil.which(args.multimon) is None:
        raise RuntimeError("multimon-ng is not installed. Run ./setup/rtl2838.sh bootstrap first")
...
def build_command(args: argparse.Namespace) -> tuple[list[str], dict[str, str] | None]:
    pipeline = (
        ...
        f"| {shlex.quote(args.multimon)} -t raw -a POCSAG1200 -a POCSAG2400 -A -"
    )
    env = None
    if "/" in args.rtl_fm:
...
def parse_pocsag_line(line: str) -> dict[str, str] | None:
    match = POCSAG_RE.match(line.strip())
    if not match:
        return None
    body = match.group("body").strip()
    text = body.split("Alpha:", 1)[1].strip() if "Alpha:" in body else body
    return {
            ...
            while True:
                ...
                if stdout_chunk:
                    ...
                    while "\n" in stdout_buffer:
                        ...
                        last_status = line
                        last_activity = current_time()
                        parsed = parse_pocsag_line(line)
                        if parsed is None:
                            bad_lines += 1
                            continue
                        now = current_time()
                        state = addresses.setdefault(parsed["address"], PagerState(address=parsed["address"]))
                        state.count += 1
                        state.last_seen = now
                        state.mode = parsed["mode"]
                        state.function = parsed["function"]
                        state.preview = parsed["text"][:90]
                        total_messages += 1
                        last_status = f"decoded {parsed['address']} fn={parsed['function']}"
                        logger.log(timestamp=now, kind="page", data=parsed)
                ...
                draw_text(
                    ...
                    0,
                    0,
                    f"Pager Monitor  freq={args.frequency / 1e6:.3f} MHz  fps={args.fps:.1f}  msgs={total_messages}  rate={total_messages / elapsed:.1f}/s  addresses={len(addresses)}",
                    curses.A_BOLD,
                )
                ...
                draw_text(
                    ...
                    monitor_status_line(
                        ...
                        idle_label="decoder alive, no accepted pages yet",
                        receiving_label="receiving",
                        detail=last_status,
                        width=width,
                    ),
                )
                draw_text(stdscr, 4, 0, "Address       Mode       Count  Last(s)  Fn  Preview")
                row = 5
                for state in sorted(addresses.values(), key=lambda item: (-item.last_seen, -item.count)):
                    age = now - state.last_seen
                    draw_text(
                        stdscr,
                        row,
                        0,
                        f"{state.address[:12]:<12}  {state.mode[:10]:<10} {state.count:>6}  {age:>7.1f}  {state.function[:2]:<2}  {state.preview[: max(0, width - 52)]}",
                    )
                    row += 1
                    if row >= height - 4 or row - 5 >= args.max_addresses:
                        break
                footer_y = height - 3
...
def main() -> int:
    ...
    try:
        ...
        print(f"rf_pager_monitor failed: {exc}", file=sys.stderr)
        return 1
